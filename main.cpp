...
void drawOption2(bool isOption2,blockchain& alfa){
    ...
    int selectedOption = 0;
    std::string options[menuOptionsCount] = {  "Monto", "Fecha" };
    Texture2D image = LoadTexture("../assets/sakura2.png");  // Reemplaza "image.png" con el nombre de tu imagen
    while (!enterKeyPressed) {
        if (IsKeyPressed(KEY_LEFT)) {
            selectedOption = (selectedOption - 1 + menuOptionsCount) % menuOptionsCount;
...
void DrawInsert(bool& shouldExit, bool& shouldMenu){
...
// Reiniciar la posición Y cuando el fondo haya pasado por completo
        ...
        std::string options[menuOptionsCount] = { "View", "Insert", "Search","Minar", "Exit" };
...
//    total.push_back(f);
//
    vector<transaccion> trans2{c};  //2 (no tiene)
    vector<transaccion> trans3{b,d};    //3
    vector<transaccion> trans4{e,f};    //4
...
//
    ...
    auto v = alfa.contains("j");
    auto s = alfa.get_block_id_from_user(v);

    cout << "uwu" <<endl;
    for (auto temp:s){
        cout << temp << endl;
        auto temp2 = alfa.find_block(temp);

    }
//        alfa.edit_block(2,1,"Yashajin_Ai",true);
//    auto v = alfa.start_with("ch");
//    auto temp = alfa.get_block_id_from_user(v);
//    cout << "get_id" << endl;
//    for (auto x:temp){
//        cout << x << endl;
//        auto t = alfa.find_block(x);
//        auto z = t->getTransaccion();
//
//
//    }

//    auto v = alfa.transaction_range_search_monto(100,700);
//    for(auto a : v){
//        a.display();
//    }
//    app::runApp(alfa);
